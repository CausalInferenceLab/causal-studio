...
- Beat 시각화 방식이 여러 가지로 가능할 때는 코드를 바로 작성하지 말고, AskUserQuestion 도구로 사용자에게 선택지를 제시한다. 선택지마다 `markdown` 필드에 ASCII 스케치(아이콘/화살표/박스 배치)를 넣어 시각적으로 비교할 수 있게 한다. 예: DAG, 분기 화살표, Before/After 카드, 산점도, 표 등 구체적인 레이아웃을 보여준다.
...
### 오디오 합성
```bash
../../.Codex/skills/manim-video-pipeline/scripts/mux_audio.sh {video.mp4} {audio.mp3} {output.mp4}
```

### 최종 합본 + BGM
```bash
cd videos/{topic} && ../../.Codex/skills/manim-video-pipeline/scripts/mix_bgm.sh \
...
```bash
cd .Codex/skills/manim-video-pipeline && \
...
../../.Codex/skills/manim-video-pipeline/scripts/mux_audio.sh \
...
### 전체 합본
```bash
cd videos/{topic} && ../../.Codex/skills/manim-video-pipeline/scripts/concat_videos.sh build/final/ build/final/{topic}_full.mp4
```
